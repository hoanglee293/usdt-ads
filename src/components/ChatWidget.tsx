    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[80vw] sm:w-96 h-[60vh] sm:h-[600px] bg-white dark:bg-gray-800 rounded-xl shadow-2xl flex flex-col border border-gray-200 dark:border-gray-700 overflow-hidden font-inter">
          ...
          {/* Input Area */}
          <div className="border-t border-gray-200 dark:border-gray-700 xs:p-2 p-1 bg-white dark:bg-gray-800">
            <div className="flex items-center xs:gap-2 gap-1">
              <input
                ref={inputRef}
                type="text"
                value={messageInput}
                ...
                className="flex-1 rounded-xl border-none outline-none xs:px-4 xs:py-2.5 px-2 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                disabled={!isConnected || messagesLoading}
              />
              <button
                onClick={handleSendMessage}
                disabled={!messageInput.trim() || !isConnected || messagesLoading}
                className="xs:w-10 xs:h-10 w-8 h-8 px-2 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-all border-none outline-none cursor-pointer"
