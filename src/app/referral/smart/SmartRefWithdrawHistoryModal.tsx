...
import { getSmartRefWithdrawHistory, GetSmartRefWithdrawHistoryResponse } from "@/services/RefService";
import Modal from "@/components/Modal";
import { useLang } from "@/lang";
import { format } from "date-fns";
import { Loader2 } from "lucide-react";

interface SmartRefWithdrawHistoryModalProps {
    isOpen: boolean;
    onClose: () => void;
}

export default function SmartRefWithdrawHistoryModal({ isOpen, onClose }: SmartRefWithdrawHistoryModalProps) {
    const { t } = useLang();

    const { data: historyData, isLoading } = useQuery<GetSmartRefWithdrawHistoryResponse>({
        queryKey: ["smartRefWithdrawHistory"],
        queryFn: getSmartRefWithdrawHistory,
        enabled: isOpen,
    });
